...
import {asyncActions as asyncAuthAction} from './../Auth/auth-reducer'


export const fetchTasks = createAsyncThunk<{ tasks: TaskType[], todolistId: string }, string, ThunkErrorType>('tasks/fetchTasks', async (todolistId, thunkAPI) => {
...
export const addTask = createAsyncThunk<TaskType, { title: string, todolistId: string }, ThunkErrorType>('tasks/addTask', async (arg, thunkAPI) => {
    thunkAPI.dispatch(setAppStatus({status: 'loading'}))
    try {
...
export const updateTask = createAsyncThunk<{ taskId: string, todolistId: string, model: UpdateDomainTaskModelType }, { id: string, domainModel: UpdateDomainTaskModelType, todolistId: string }, ThunkErrorType>('tasks/updateTask', async (arg, thunkAPI) => {
    ...
    if (!task) {
        return thunkAPI.rejectWithValue({errors: ['task not found in the state'], fieldsErrors: undefined})
    }
